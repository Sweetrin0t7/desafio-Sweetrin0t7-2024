class RecintosZoo {
    ...
    //3 - Animais já presentes no recinto devem continuar confortáveis com a inclusão do(s) novo(s)
        ...
        if (tipoAnimal === 'HIPOPOTAMO' && recintoAtual.animaisExistentes.length > 0 && !(recintoAtual.bioma.includes('savana') && recintoAtual.bioma.includes('rio'))) {
          console.log(`Recinto não é adequado (Hipopótamos só toleram outras espécies em recintos com savana e rio)`);
          return;
        }
        if (recintoAtual.animaisExistentes.includes('HIPOPOTAMO') && tipoAnimal != 'HIPOPOTAMO') {
          console.log(`Recinto não é adequado (Hipopótamos só toleram outras espécies em recintos com savana e rio)`);
          return;
        }
